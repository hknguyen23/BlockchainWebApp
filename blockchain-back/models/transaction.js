...
const validateTransaction = (transaction, aUnspentTxOuts) => {
  ...
  const validAddressTxOutValues = transaction.txOuts
    .map((txOut) => isValidAddress(txOut.address))
    .reduce((a, b) => a && b, true);
...
const hasDuplicates = (txIns) => {
  const groups = _.countBy(txIns, (txIn) => txIn.txOutId + txIn.txOutIndex); // perform callback to txIns then count duplicate
  return _(groups)
    .map((value, key) => {
      if (value > 1) {
